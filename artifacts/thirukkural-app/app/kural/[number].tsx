...
export default function KuralDetailScreen() {
  ...
  const kural = kurals.find((k) => k.number === kuralNumber);
  const currentIndex = kurals.findIndex((k) => k.number === kuralNumber);
  const prevKural = currentIndex > 0 ? kurals[currentIndex - 1] : null;
  const nextKural = currentIndex < kurals.length - 1 ? kurals[currentIndex + 1] : null;
    ...
    <View style={[styles.container, { paddingTop: topPadding }]}>
        ...
        >
          <View style={styles.verseCard}>
            <View style={styles.verseCardHeader}>
              ...
              <View style={styles.palBadge}>
                <Text style={styles.palBadgeText}>கடவுள் வாழ்த்து</Text>
              </View>
            </View>
          ...
          <View style={styles.metaSection}>
            <View style={styles.metaItem}>
              <Text style={styles.metaLabel}>அதிகாரம்</Text>
              <Text style={styles.metaValue}>{data.pals[0].adhikarams[0].name}</Text>
            </View>
            <View style={styles.metaDivider} />
            <View style={styles.metaItem}>
              <Text style={styles.metaLabel}>பால்</Text>
              <Text style={styles.metaValue}>{data.pals[0].name}</Text>
            </View>
            <View style={styles.metaDivider} />
            <View style={styles.metaItem}>
              <Text style={styles.metaLabel}>உரையாசிரியர்</Text>
              <Text style={styles.metaValue} numberOfLines={2}>{data.book.commentaryAuthor}</Text>
            </View>
          </View>
        </Animated.View>
      </ScrollView>
      ...
      <View style={[styles.navBar, { paddingBottom: bottomPadding + 8 }]}>
        <Pressable
          ...
          style={({ pressed }) => [
            styles.navButton,
            !prevKural && styles.navButtonDisabled,
            pressed && { opacity: 0.7 },
          ]}
        >
          <Ionicons
          ...
          />
          <Text style={[styles.navButtonText, !prevKural && styles.navButtonTextDisabled]}>
            {prevKural ? `குறள் ${prevKural.number}` : "முதல் குறள்"}
          </Text>
        </Pressable>
        ...
        <View style={styles.navDots}>
          {kurals.map((k) => (
            <View
              ...
              style={[
                styles.navDot,
                k.number === kuralNumber && styles.navDotActive,
              ]}
            />
          ))}
        </View>
        ...
        <Pressable
          ...
          style={({ pressed }) => [
            styles.navButton,
            !nextKural && styles.navButtonDisabled,
            pressed && { opacity: 0.7 },
          ]}
        >
          <Text style={[styles.navButtonText, !nextKural && styles.navButtonTextDisabled]}>
            {nextKural ? `குறள் ${nextKural.number}` : "கடைசி குறள்"}
          </Text>
          <Ionicons
